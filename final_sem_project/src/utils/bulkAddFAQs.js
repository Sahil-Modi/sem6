...
export const FAQ_KB = defaultFAQs;

export const bulkAddFAQs = async () => {
  let successCount = 0;
  let errorCount = 0;
  const errors = [];

  console.log('🚀 Starting bulk FAQ addition...');
  console.log(`📋 Total FAQs to add: ${defaultFAQs.length}`);
  ...
  for (const faq of defaultFAQs) {
    try {
      ...
      successCount++;
      console.log(`✅ Added (${successCount}/${defaultFAQs.length}): ${faq.question.substring(0, 50)}...`);
    } catch (error) {
      errorCount++;
      errors.push({ question: faq.question, error: error.message });
  ...
  if (errors.length > 0) {
    console.log('\n⚠️ Errors:');
    errors.forEach(err => console.log(`  - ${err.question}: ${err.error}`));
  }
...
if (typeof window !== 'undefined') {
  window.bulkAddFAQs = bulkAddFAQs;
}

export default bulkAddFAQs;
